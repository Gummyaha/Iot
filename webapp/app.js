...
function getStatisticEntrance(){
    $.get( "http://35.198.217.40/api/debug", function( data ) {
    $( ".result" ).html( data );
      statisticEntrance_data = [20,45,30,10,25,50,40,15,45,30,
                                35,10,40,25,50,20,15,22,30,16,
                                48,12,36,50]
      test_chart()
    });
}
     ...
     setTimeout(function(){
        $.ajax({ url: "http://35.198.217.40/api/jenny/entrance",
          success: function(data){
            ...
            entrance_plate.innerText = entrance_data[0]['plate']
            entrance_plate = document.getElementById('entrance_time_info')
            entrance_plate.innerText = entrance_data[0]['time']
  ...
  setTimeout(function(){
     $.ajax({ url: "http://35.198.217.40/api/jenny/exit",
       success: function(data){
         ...
         exit_plate.innerText = entrance_data[0]['plate']
         exit_plate = document.getElementById('exit_time_info')
         exit_plate.innerText = entrance_data[0]['time']
